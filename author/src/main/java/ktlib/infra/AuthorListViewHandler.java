...
public class AuthorListViewHandler {
    ...
    public void whenRequestedPublish_then_UPDATE_1(
        ...
        try {
            ...
            // view 객체 조회
            Optional<AuthorList> authorListOptional = authorListRepository.findByAuthorId(
                requestedPublish.getAuthorId()
            );

            if (authorListOptional.isPresent()) {
                AuthorList authorList = authorListOptional.get();
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                authorList.setTitle(requestedPublish.getTitle());
                // view 레파지 토리에 save
                authorListRepository.save(authorList);
            }
        } catch (Exception e) {
